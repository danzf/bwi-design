...
export default {
    ...
    iconClasses () {
      ...
      if (!this.isCustomSize) {
        Object.assign(classes, {
          'bwi-icon--xs': this.size === 'xs',
          'bwi-icon--sm': this.size === 'sm',
          'bwi-icon--md': this.size === 'md',
          'bwi-icon--lg': this.size === 'lg',
          'bwi-icon--xl': this.size === 'xl',
        })
      }
      return classes
    },
  ...
  methods: {
    async loadIcon (name) {
      ...
      const iconComponent = await import(`@danzf/bwi-design/icons/${name}?vue`)
        .then((m) => m.default || m)
